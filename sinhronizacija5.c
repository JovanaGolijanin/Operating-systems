...
int N = 0; /*Globalna promenljiva koja predstavlja brojac citaoca koji u datom
trenutku koriste bazu podataka*/
...
void * citalac(void * arg)
{
	...
	while(1)
	{
		...
		N++;
		/*Prvi citalac zakljucava bazu podataka kako bi sprecio pisca da joj
		pristupa*/
		if (N == 1)
			sem_wait(&mutex_DB);
		sem_post(&mutex_N);
